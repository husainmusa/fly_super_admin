      ...
      if (data && data.status === 200 && data.data.length > 0) {
        ...
        if (((x) => { try { JSON.parse(x); return true; } catch (e) { return false } })(info.orders)) {
          if (((x) => { try { JSON.parse(x); return true; } catch (e) { return false } })(info.notes)) {
            ...
            this.api.post('acceptedorders/getByOrderId', orderParam).then((accepteddata: any) => {
                console.log("accepted driver data", accepteddata.data[0]);
                let driverIdArray = this.driverId.split(',');
                console.log("all drivers", driverIdArray);
                if (accepteddata && accepteddata.status === 200 && accepteddata.data.length > 0) {
                  for(let i = 0; i < driverIdArray.length; i++){
                    if(driverIdArray[i] == accepteddata.data[0].driver_id){
                      this.acceptedDriver = driverIdArray[i];
                    }
                  }
                  console.log("driverid", this.acceptedDriver);
                  if (this.acceptedDriver && this.acceptedDriver !== '') {
                    const userinfo = {
                      id: this.acceptedDriver
                    };
