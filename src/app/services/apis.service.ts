...
export class ApisService {
  ...
  /*========= add object in Firebase ===========*/
  createObjectInFirebase(path,key){
    const itemRef = this.firbaseDB.object([path,key].join('/'));
  ...
  sendNotification(msg, title,cover?) {
    const body = {
      app_id: environment.onesignal.appId,
      included_segments: ['Active Users', 'Inactive Users"'],
      headings: { en: title },
      contents: { en: msg },
      big_picture:this.mediaURL+cover,
      data: { task: msg }
    };
    const header = {
